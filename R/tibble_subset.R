...
tibble_subset <- function (tib, var, tempDir) {
  ...
  if (!any(is.na(categories))) {
    ...
    if (length(categories) == 1){
      ...
      write(paste0(stringr::str_to_sentence(gsub("_", " ", names(var))), ": ", categories), file = paste(tempDir, "selected_hf.txt", sep = "/"), append = TRUE)
      return(tib)
    } else {
      cat("\n")
      ...
      if (length(selInd) == 0){
        write(paste0(stringr::str_to_sentence(gsub("_", " ", names(var))), ": ", paste(categories, collapse = ", ")), file = paste(tempDir, "selected_hf.txt", sep = "/"), append = TRUE)
        return(tib)
        ...
        write(paste0(stringr::str_to_sentence(gsub("_", " ", names(var))), ": ", paste(categories[selInd], collapse = ", ")), file = paste(tempDir, "selected_hf.txt", sep = "/"), append = TRUE)
        return(tib)
      }
    }
  } else {
    ...
    if (all(is.na(categories))) {
      ...
      if (yn == 1) {
        write(paste0(stringr::str_to_sentence(gsub("_", " ", names(var))), ": NA"), file = paste(tempDir, "selected_hf.txt", sep = "/"), append = TRUE)
        return(tib)
      }else{
