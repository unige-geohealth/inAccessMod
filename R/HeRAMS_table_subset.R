...
HeRAMS_table_subset <- function (tibT, tibC, varCol, stopQuest = TRUE, codeName = NULL, stopLst = NULL, tempDir, barriers, suffix, impairmentValues) {
  ...
  categories[!complete.cases(categories)] <- "Not available"
  print(categories)
  ...
  categories[categories == "Not available"] <- NA
  ...
  if (length(colBarriers) == 0) {
    ...
    if (any(impair) & barriers){
      ...
      write(paste0(varCol, "_", substr(suffix, 1, 1), " -> ", paste(categories, collapse = " + ")), file = paste(tempDir, "selected_hf.txt", sep = "/"), append = TRUE)
    }
  }
  return(list(tibT, tibC))
}
