...
Let's say that we would like to re-run the function with different selection criteria. For instance, we select only the "Hospital" type.
...
We are asked to validate the column names for "external id", workspace id", "date", "HF type" and "HF name".
...
```{r cov, eval=FALSE}
hf_best_cov(workDir = "C:/Test/BestCoverage/", catchShp = "catchment", popRaster = "population.tif", catchHfColName = "facilityName", nTot = 10)
```
...
```{r cov2, eval=FALSE}
hf_best_cov(workDir = "C:/Test/BestCoverage/", catchShp = "catchment", popRaster = "population.tif", catchHfColName = "facilityName", nTot = 10, adminCheck = TRUE, hfShp = "facilities", hfHfColName = "facilityName", npAdmin = 1, adminShp = "adminUnit", adminColName = "regionName")
```
