...
country <- "SWITZERLAND"
copy_input(mainPath, country, "C:/example/boundaries")
```

We are then asked to select the data we want to copy.
...
HeRAMS_filter_hf(mainPath, country, pathTable = NULL, barriers = FALSE, mostRecentObs = TRUE)
```
...
HeRAMS_filter_hf(mainPath, country, pathTable = NULL, scenario = NULL, mostRecentObs = TRUE)
```
