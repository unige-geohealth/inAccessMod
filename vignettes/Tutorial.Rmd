...
Then we use its *install_github* function to download and install the **inAccMod** package from its github repository.

```{r install, eval=FALSE}
devtools::install_github("ptimoner/inAccMod")
```
...
Now, let's filter the health facilities from our raw health facility table with the *filter_hf*  function and create a point shapefile of the filtered facilities with the *create_hf_facilities* function. This section is compatible with the HeRAMS Excel tables. If we are not working with HeRAMS data, we can just skip this part and use the *copy_input* function to copy a health facility shapefile into an appropriate sub-folder (it will copy the input to e.g. /vFacilities/20220524103021/raw).

The function *filter_hf* requires the main project folder and the country folder name. Besides, it also requires the path to the raw Excel table (if NULL, we are asked whether we want to take fictitious example data from Switzerland). Three other parameters are optional: scenario, barriers and mostRecentObs.
...
```{r fictitious}
inAccMod::fictitious_herams_data_txt[1:6, c("last_synced", "subject_id", "date", "MoSD4", "CONDB", "HFFUNCT", "HFACC", "MoSDGPS_SQ001", "MoSDGPS_SQ002")]
```
...
Let's have a loot at the log.txt file
